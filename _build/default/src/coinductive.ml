(* Projection from global automaton to local automaton using knowledge sets *)
...
(* Knowledge set: states the participant considers possible *)
...
type step_result =
  | Stop
  | SendStep of role * base * knowledge option
  | RecvStep of role * base * knowledge option
  | IntStep  of role * (label * knowledge option) list
  | ExtStep  of role * (label * knowledge option) list

(* Check if participant is involved in state *)
...
(* Successor states excluding transitions involving p *)
...
(* Compute knowledge set: states involving p reachable without p-transitions *)
